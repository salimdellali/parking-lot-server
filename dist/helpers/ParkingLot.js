...
class ParkingLot {
    ...
    constructor(parkingLotSize) {
        if (!isPositiveNoneNullNumber(parkingLotSize))
            throw new Error('Invalid Parking lot size input, please provide a positive and non null number in the .env file and restart the app');
        let parking = {};
    ...
    isParkingLotFull() {
        for (const slotId in this.parkingLot) {
            if (this.parkingLot[slotId] === null)
                return false;
        }
        return true;
    }
    ...
    getParkingSlot(carId) {
        for (const slotId in this.parkingLot) {
            if (this.parkingLot[slotId] === carId)
                return slotId;
        }
        return null;
    }
     ...
     * @returns {ParkingSlotInformation} parking slot information
     */
    getSlotInformationByCarId(carId) {
        const slotId = Object.keys(this.parkingLot).find((key) => this.parkingLot[key] === carId);
        return {
            slotId,
            carId,
        };
