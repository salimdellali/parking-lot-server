...
const { getCurrentUnixTimeStampInSeconds, diffTimeStampsInSeconds, } = require('../helpers/times');
...
const rateLimiter = (req, res, next) => {
    ...
    if (typeof currentRequestLog === 'undefined' ||
        diffTimeStampsInSeconds(currentRequestTimeStampInSeconds, currentRequestLog.firstRequestTimeStampInSeconds) >= WINDOW_SIZE_IN_SECONDS) {
        const newRequestLog = {
    ...
    // else if there are remaining tokens, decrement tokens by 1, and allow request
        ...
        const upatedRequestLog = Object.assign(Object.assign({}, currentRequestLog), { tokens: currentRequestLog.tokens - 1 });
        cache.setRequestLog({
            ip: currentRequestIp,
    ...
    // no tokens remaining within the window, deny requests
        ...
        const timeRemaining = WINDOW_SIZE_IN_SECONDS -
            diffTimeStampsInSeconds(currentRequestTimeStampInSeconds, currentRequestLog.firstRequestTimeStampInSeconds);
        res.statusCode = 429;
        res.setHeader('Retry-After', timeRemaining);
        res.send(`You have exceeded the ${MAX_WINDOW_REQUEST_COUNT} requests in ${WINDOW_SIZE_IN_SECONDS}s limit. Retry after ${timeRemaining}s`);
    }
};
