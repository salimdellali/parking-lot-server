...
const { isCarIdValid, isSlotIdValid, getPrefix, } = require('../helpers/utilities');
...
router.get('/:id', rateLimiter, (req, res) => {
    ...
    if (!isCarIdValid(id) && !isSlotIdValid(id))
        return res
            .status(400)
            .send("Incorrect car ID or slot ID, please submit a correct ID in a format like 'car_#####' or a correct slot ID in a format like 'slot_#####' where # is a digit");
